    ...
    private func loadImages(for scrollViewPage: Int) {
        ...
        for i in 0..<totalCount {
            ...
            case .fixed(let offset):
                ...
                let absValue = abs(scrollViewPage - i)
                let shouldLoad = absValue <= offset || absValue > (totalCount - offset) || circularEdgeLoad
...
extension ImageSlideshow: UIScrollViewDelegate {
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if circular {
            ...
            if scrollView.contentOffset.x >= (regularContentOffset + regularContentOffset) {
