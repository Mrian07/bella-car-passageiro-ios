...
class ExeServerUrl: NSObject {
    ...
    */
    private func continuePostProcess(postString:String, request: NSMutableURLRequest, completionHandler: @escaping CompletionHandler){
        ...
        if (UserDefaults.standard.object(forKey: Utils.SERVICE_CATEGORY_ID) != nil && GeneralFunctions.getValue(key: Utils.SERVICE_CATEGORY_ID) as! String != ""){
            deliveryAllCategoryID = GeneralFunctions.getValue(key: Utils.SERVICE_CATEGORY_ID) as! String
        }
        
        if (UserDefaults.standard.object(forKey: Utils.DEFAULT_SERVICE_CATEGORY_ID) != nil && GeneralFunctions.getValue(key: Utils.DEFAULT_SERVICE_CATEGORY_ID) as! String != ""){
        ...
        var postString = postString + "&tSessionId=\(GeneralFunctions.getSessionId())&GeneralMemberId=\(GeneralFunctions.getMemberd())&GeneralUserType=\(Utils.appUserType)&GeneralDeviceType=\(Utils.deviceType)&GeneralAppVersion=\(Utils.applicationVersion())&vTimeZone=\(DateFormatter().timeZone.identifier)&vUserDeviceCountry=\(Utils.getDeviceCountryCode())&iServiceId=\(deliveryAllCategoryID)&DEFAULT_SERVICE_CATEGORY_ID=\(defaultDeliveryAllCategoryID)&vCurrentTime=\(nowDate)&deviceHeight=\(((Application.screenSize.height - GeneralFunctions.getSafeAreaInsets().top) * UIScreen.main.scale))&deviceWidth=\(Application.screenSize.width * UIScreen.main.scale)&APP_TYPE=\(CUSTOM_APP_TYPE)&UBERX_PARENT_CAT_ID=\(CUSTOM_UBERX_PARENT_CAT_ID)"
